...
import ru.ok.technopolis.training.personal.utils.Navigator
            ...
            if (hasNavigationMenu()) {
                ...
                navMenu?.setOnDrawerItemClickListener { _, _, _, _, drawerItem ->
                    when(drawerItem.identifier) {
                        SEARCH_ITEM_ID -> Navigator.onSearchItemClicked()
                        BOOKMARKS_ITEM_ID -> Navigator.onBookmarksItemClicked()
                        FAVOURITE_ITEM_ID -> Navigator.onFavouriteItemClicked()
                        SETTINGS_ITEM_ID -> Navigator.onSettingsItemClicked(this)
                    }
                }
            } else {
